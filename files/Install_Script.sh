...
if [ -e "${CHANGELOG}" ]
	then
		...
		echo "WARNING ! ts3server is out of date ... will download new copy from teamspeak."
			...
			wget --no-cache --show-progress --progress=bar:force:noscroll https://files.teamspeak-services.com/releases/server/${TS_VERSION}/teamspeak3-server_linux_alpine-${TS_VERSION}.tar.bz2 -O /ts3temp/ts3server_${TS_VERSION}.tar.bz2
